...
<script>
    ...
    contactForm.addEventListener('submit', async (event) => {                    // event.preventDefault() [use for prevent reload]
        ...
        if (name.length === 0) {
            ...
            let formData = {                                     // This is for sending data to back-end
                fullName: name,                                // 1st fullName = property, 2nd name = value
                email: email,
                phone: phone,
                message: message
            }
            ...
            if(result.status===200 && result.data===1){
                alert('You request has submitted successfully');
                contactForm.reset();
            }
            else {
